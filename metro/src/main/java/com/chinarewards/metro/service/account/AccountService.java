...
public class AccountService implements IAccountService {
	...
	protected void withdrawal(String token, Account account, String unitCode,
		...
		for (AccountBalanceUnits abu : accBalanceUnits) {
			...
			if (balance > value) {
				...
				Ledger ledger = new Ledger(abu.getExpDate(), unitCode,
						abu.getUnitPrice(), -balance, tx, account);
				hbDaoSupport.save(ledger);
	...
	public void withdrawal(String token, Account account, Unit unit,
		...
		withdrawal(token, account, unit.getUnitCode(), unit, value, tx,
				expiryDate);
	}
	...
	public List<SoonExpireAccountBalanceVo> soonExpireAccountBalanceByMember(
		...
		page.setPage(1);
		page.setRows(10);
		List<SoonExpireAccountBalanceVo> list = hbDaoSupport
				.executeQuery(
						"SELECT new com.chinarewards.metro.model.member.SoonExpireAccountBalanceVo(SUM(abu.units) as units, abu.expDate AS expDate)"
								+ " FROM AccountBalanceUnits abu WHERE abu.accountBalance.account=:account AND abu.unit.unitCode=:unitCode AND abu.expired=:expired AND abu.tx.status=:status GROUP BY DATE_FORMAT(abu.expDate, '%Y-%m') ORDER BY DATE_FORMAT(abu.expDate, '%Y-%m') ASC ",
						map, page);
	...
	public List<AccountBalanceUnits> findAccountBalanceUnits(Date fromDate,
		...
		str.append("FROM AccountBalanceUnits a WHERE a.accountBalance.unitCode=:unitCode AND a.expired = :expired");
		Map<String, Object> params = new HashMap<String, Object>();
		if (fromDate != null) {
		...
		str.append(" AND NOT EXISTS (SELECT m.account FROM Member m WHERE m.status=:memberStatus AND m.account = a.accountBalance.account.id)");
		params.put("memberStatus", Dictionary.MEMBER_STATE_LOGOUT);
		
		params.put("expired", false);
		params.put("unitCode", Dictionary.UNIT_CODE_BINKE);
		List<AccountBalanceUnits> list = hbDaoSupport.findTsByHQLPage(
				str.toString(), params, page);
		return list;
	}
	...
	@Override
	public Transaction findTransaction(String token, String txId) {
		return hbDaoSupport.findTById(Transaction.class, txId);
	}

	@Override
	public Transaction unfrozenPosSales(String token, String txId) {
		Transaction tx = hbDaoSupport.findTById(Transaction.class, txId);
		List<TransactionQueue> queues = hbDaoSupport.findTsByHQL(
				"FROM TransactionQueue q WHERE q.tx.txId=? AND q.tx.status=?",
				txId, TxStatus.FROZEN);

		for (TransactionQueue tq : queues) {
			Unit unit = hbDaoSupport.findTByHQL(
					"FROM Unit u WHERE u.unitCode=?", tq.getUnitCode());
			if (tq.getType() == 0) { // 存
				deposit("0", tq.getAccount(), unit,
						tq.getUnits().doubleValue(), tx);
			} else if (tq.getType() == 1) { // 取
				withdrawal("0", tq.getAccount(), unit, tq.getUnits()
						.doubleValue(), tx);
			}
		}
		tx.setStatus(TxStatus.COMPLETED);
