...
public class MyFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource{
	...
	public Collection<ConfigAttribute> getAttributes(Object object)throws IllegalArgumentException {
		...
		ConfigAttribute config_ = new SecurityConfig("ACCESS_DENIED");
		Collection<ConfigAttribute> noPermission = new ArrayList<ConfigAttribute>();
		noPermission.add(config_);
		
		final HttpServletRequest request = ((FilterInvocation) object).getRequest();
		String url = request.getServletPath();
		if(!(path+"authorize").equals(url)){
			if(request.getSession().getAttribute(UserContext.USER_ID)==null) return noPermission;
		}
		if(!isDbUrl(url)) return returnCollection;
		boolean isAuth = checkUserAuth(request,url);
		if(isAuth){
			...
			if(res.getOpertions().size() > 0){
				for(ResourceOperation operation : res.getOpertions()){
					if(operation.getUrl() != null){
						String urls[] = operation.getUrl().split(",");
						for(String url_ : urls){
							if(url.equals(path+url_)){
								flag = true;break;
							}
						}
					}
	...
	public boolean checkUserAuth(HttpServletRequest request,String url){
		...
		if(list.size() == 0){
			list = userService.findUserResources();
		}
