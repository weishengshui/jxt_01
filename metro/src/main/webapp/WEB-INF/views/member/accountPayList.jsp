...
<script>
	function edit(id_,name_){
		...
		if(id == undefined){
			...
			id = row.id;
			name = decodeURIComponent(row.surname+row.name);
			if(name == 0){
				name = row.phone;
			}

		}
		parent.addTab(name+'账户充值','member/accountPay?id='+id);
	}
...
<body>
	...
	<div id="toolbar">  
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="edit()">充值</a>  
    </div>
	<!-- 显示列表Table -->
	<table id="table" class="easyui-datagrid" data-options="url:'findMemebers',fitColumns:true,striped:true,loadMsg:'正在载入...',pagination:true,toolbar: '#toolbar',
		rownumbers:true,pageList:pageList,singleSelect:true,onDblClickRow:function(rowIndex,rowData){edit(edit(rowData.id,rowData.name==null?rowData.phone:rowData.surname+rowData.name));}">
	    <thead>  
	        <tr>  
