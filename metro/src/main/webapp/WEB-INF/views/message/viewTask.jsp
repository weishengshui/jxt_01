...
$(function(){
	...
	if(senddate.length==0){
	 ...
	 if(${task.taskStates}!=2||'${task.failureAmount}'==0){
...
<body>
	...
	<div id="tt" class="easyui-tabs" style="margin:10px;" > 
     
            <table  border="0" style="padding: 20px;">
			<tr>
				<td width="100px"><span style="color: red;">*</span>任务名称:</td>
			...
			<tr>
				<td  width="100px">
				    <input type="radio" id="immedSend" name="sendType" value="1" disabled="true" /> 立即发送
				</td>
			</tr>
			<tr>
				<td  width="150px">
				    <input type="radio" id="planSend" name="sendType" value="2" disabled="true" /> 计划发送时间 
				</td>
			    <td>
	...
	<div>${task.taskName}任务--所有电话列表：</div>
	  <table id="atable" class="easyui-datagrid" data-options="rownumbers:true,url:'listTel?taskid=${task.taskId}',fitColumns:true,striped:true,loadMsg:'正在载入...',pagination:true,
	    ...
	    <thead>  
	        <tr>  
	            <th data-options="field:'telephone'">电话号码</th>
	           <th data-options="field:'sendTime',formatter:function(v){return dateFormat(v)}">发送时间</th>
	            <th data-options="field:'states',formatter:function(v){return sendResult(v)}">发送结果</th>
	        </tr>   
	    </thead>  
	 </table> 
	</div>
