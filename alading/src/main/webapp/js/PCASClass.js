...
PCAS.SetC = function(PCA) {
	...
	PCA.SelC.length = 0;
	PCA.SelC.options.add(new Option(SCT, '')); // edited add row
	for (i = 1; i < PCAC[PI].length; i++) {
		...
		if (PCA.DefC == PCACV) PCA.SelC[i].selected = true //// edited   del "- 1"   PCA.SelC[i - 1 ]
	}
	if (PCA.SelA) PCAS.SetA(PCA)
};
PCAS.SetA = function(PCA) {
	PI = PCA.SelP.selectedIndex;
	CI = PCA.SelC.selectedIndex - 1;      // edited   add "- 1"
	PCA.SelA.length = 0;
	PCA.SelA.options.add(new Option(SAT, '')); // edited  add row
	if( CI < 0 ) return;						// edited add row
	for (i = 1; i < PCAA[PI][CI].length; i++) {
		PCAAT = PCAAV = PCAA[PI][CI][i];
		if (PCAAT == SAT) PCAAV = "";
		PCA.SelA.options.add(new Option(PCAAT, PCAAV));
		if (PCA.DefA == PCAAV) PCA.SelA[i].selected = true ////edited del "-1"  PCA.SelA[i - 1]
	}
}
...
$(function(){
	$('#province').combobox({
        onChange:function(){
        	...
        	$("#province option").each(function(i){
        		if($("input[name='province']").val() == $(this).val()){
        			index = i;
        			pid = i;
        		}
        	});
    		...
    		arr["text"] = SCT;
    		arrs[0] = arr; 
        	for (i = 1; i < PCAC[index].length; i++) {
        		var arr = {};
        		arr["id"] = PCAC[index][i];
        		arr["text"] = PCAC[index][i];
        		if(PCAC[index][i] == SCT){
        			continue;
        		}
        		arrs[i] = arr;
	...
	$('#city').combobox({
        onChange:function(){
        	var index = 0;
        	var datas = $("#city").combobox('getData');
        	
        	for(var i=0;i<datas.length;i++){
        		if($("input[name='city']").val() == datas[i].text){
        			index = i ;break;
        		}
        	}
        	var arrs = new Array();
    		...
    		if(index > 0){
    			for (i = 1; i < PCAA[pid][index-1].length; i++) {
            		var arr = {};
            		arr["id"] = PCAA[pid][index-1][i];
            		arr["text"] = PCAA[pid][index-1][i];
            		arrs[i] = arr;
            	}
    			$('#area').combobox({  
            	 	valueField: 'id',  
