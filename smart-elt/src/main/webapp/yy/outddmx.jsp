...
if (session.getAttribute("xtczqx")==null || session.getAttribute("xtczqx").toString().indexOf("5002")==-1)
{
	...
	try
	{
		strsql="select d.ddh,y.ygxm,d.shdzxx,s.spmc,x.sl from tbl_ygddzb d inner join tbl_ygddmx x on d.nid=x.dd inner join tbl_sp s on x.sp=s.nid inner join tbl_qyyg y on d.yg=y.nid where d.state in (1,11) ";
		...
		while(rs.next())
		{
			...
			for (int j = 1; j < 7; j++) {
		     ...
		     if (j==2)
		     {
		    	 ...
		    	 cell.setCellValue(rs.getString("ddh"));
		     }
		     if (j==3)
		     {
		    	 cell.setCellType(HSSFCell.CELL_TYPE_STRING);
		    	 cell.setCellValue(rs.getString("ygxm"));
		     }
		     if (j==4)
		     {
		    	 cell.setCellType(HSSFCell.CELL_TYPE_STRING);
		    	 String shdzxx=rs.getString("shdzxx").replace("<b>","");
		    	 shdzxx=shdzxx.replace("</b>","");
		    	 shdzxx=shdzxx.replace("<br />","");
		    	 cell.setCellValue(shdzxx);
		     }
		     if (j==5)
		     {
		    	 cell.setCellType(HSSFCell.CELL_TYPE_STRING);
		...
		rs.close();
		  ...
		  OutputStream os;
		  String fileName = "订单明细"+sf.format(Calendar.getInstance().getTime())+".xls";
