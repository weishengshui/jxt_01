...
{	
	response.sendRedirect("zzxianxiafukuan.jsp");
	return;
}
%>
...
try{
	...
	{
		//积分账户加上积分，修改充值单 数据
		int nzzzt=0,qy=0,zztype=0,zzr=0,bmxz=0;
		Double nzzje=0.0;
		strsql="select qy,zzje,zzzt,zztype,zzr,bmxz from tbl_jfzz where nid="+zzid;
		...
		if (nzzzt==2)
		{
			...
			if (zztype==0) {
				...
				if (zztype==2) {
					fflx=2;
					strsql="select nid,xzmc as mc from tbl_qyxz where nid="+bmxz;
				} else {
					fflx=1;
					strsql="select nid,bmmc as mc from tbl_qybm where nid="+bmxz;
				}
				rs=stmt.executeQuery(strsql);
				String mc="";
				if (rs.next())
				...
				strsql="insert into tbl_jfffxx (qy,jfff,fflx,lxbh,jf,ldbh,jsmc) values("+qy+","+jfffId+","+fflx+","+bmxz+","+zzjf+","+zzr+",'"+mc+"')";
				stmt.executeUpdate(strsql);
			}
		}
    ...
    <td bgcolor="#f4f4f4"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td><div class="local"><span>积分管理&gt; 线下订单</span></div></td>
          </tr>
          <tr>
            <td>
					<div class="caxunbox">
					<div class="caxun" style="padding: 10px;">
						...
						确认付款日期：<input type="text"  style="width: 80px;" name="sfkrj" id="sfkrj" value="<%=sfkrj%>"  onclick="new Calendar().show(this);" readonly="readonly" />--<input type="text"  style="width: 80px;" name="efkrj" id="efkrj" value="<%=efkrj%>"  onclick="new Calendar().show(this);" readonly="readonly" />										
          ...
          <tr>
            ...
            <%
            	...
            	strsql="select count(z.nid) as hn from tbl_jfzz z left join tbl_qy q on z.qy=q.nid  where z.zzfs=1";
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td><table width="100%" border="0" cellspacing="1" cellpadding="1" class="maintable">
                  ...
                  <%
                  strsql="select z.nid, z.zzsj,q.qymc,z.zzbh,z.zzje,z.zzjf,z.zzbz,z.zzzt,z.fksj,y.ygxm,q.khjl from tbl_jfzz z left join tbl_qy q on z.qy=q.nid left join tbl_qyyg y on z.zzr=y.nid  where z.zzfs=1";
             		if (qymc!=null && qymc.length()>0)
             			strsql+=" and q.qymc like '%"+qymc+"%'";
             		if (zzbh!=null && zzbh.length()>0)
             			strsql+=" and z.zzbh like '%"+zzbh+"%'";
                  ...
                  <tr>
                  	...
                  	<td><%if (rs.getString("fksj")!=null) out.print(sf.format(rs.getTimestamp("fksj"))); else out.print("");%></td>             	
                    ...
                    <td><%if (rs.getInt("zzzt")==2) out.print("资金未到账");
                    if (rs.getInt("zzzt")==3) out.print("资金到账");
                   %></td>
                    <td><%if (rs.getInt("zzzt")==2) {%><a href="javascript:void(0);" onclick="wcfk(<%=rs.getString("nid")%>)" class="blue">确认付款</a><%}%></td>
                  </tr>
                 <%}
                  rs.close();
                  %>
