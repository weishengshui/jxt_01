...
try
{
 ...
 %>
	...
	if (mm1!=null && mm1.length()>0)
	{
		...
		//判断积分是否够
		strsql="select nid from tbl_jfffxx where nid="+xid+" and jf-yffjf>="+ctjf;		
...
// 							Template template = Velocity.getTemplate("templates/mail/jfreceive.vm");
			...
			//更新已发放积分
			strsql="update tbl_jfffxx set yffjf=yffjf+"+Integer.valueOf(tjf);
			//定时发放
			//if (ffzt==0)
	...
	<div id="main">
	  	<div class="main2">
	  		<div class="box">				
				...
				<%strsql="select jf-yffjf as hjf from tbl_jfffxx where nid="+xid;
				rs=stmt.executeQuery(strsql);
				if (rs.next())
				{
					%>
					<div class="ffcg-tishi">本名目还有余额 <span><%=rs.getInt("hjf")%></span> 积分，您还可以</div>
					<%
				}
				rs.close();
				%>
