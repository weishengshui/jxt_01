...
<div class="jftable-t">
	<%
		if("fmfl".equals(request.getParameter("flag"))){
	%>
...
try
{
	...
	//strsql="select count(*) as hn from (select m.jfq from tbl_jfqddmc m inner join tbl_jfq q on m.jfq=q.nid where m.qy="+session.getAttribute("qy")+" and m.zt=1 and (m.sl<>m.ffsl or m.djsl>0)";
	...
	//strsql="select m.jfq,sum(m.sl-m.ffsl) as sysl,sum(m.djsl) as djsl,q.mc,q.jf from tbl_jfqddmc m inner join tbl_jfq q on m.jfq=q.nid where m.qy="+session.getAttribute("qy")+" and m.zt=1 and (m.sl<>m.ffsl or m.djsl>0)";
	if("fmfl".equals(flag)){
		strsql="select m.jfq,sum(m.sl-m.ffsl) as sysl,sum(m.djsl) as djsl,q.nid sid,q.hd hid,q.mc,q.jf from tbl_jfqddmc m inner join tbl_jfq q on m.jfq=q.nid where m.qy="+session.getAttribute("qy")+" and m.zt=1 and m.sl<>m.ffsl and q.yxq >= curdate()";
	}else{
	...
	if (jfqmc!=null && jfqmc.length()>0)
		strsql+=" and q.mc like '%"+jfqmc+"%'";
	strsql+=" group by m.jfq,q.mc,q.jf limit " + (Integer.valueOf(pno)-1)*psize+","+psize;
