...
<script type="text/javascript">
	...
	function showcc(t,p)
	{
		...
		if (t==3)
			{				
				...
				document.getElementById("xchild"+p).innerHTML="";
				thisp=p;
				var url = "selectyg.jsp?t=0&xid=<%=xid%>&time="+timeParam;	
				xmlHttp.open("GET", url, true);
	...
	{
		var timeParam = Math.round(new Date().getTime()/1000);
		var url = "getyglist.jsp?t="+t+"&pno="+p+"&dsygxm="+encodeURI(escape(document.getElementById("dsygxm").value))+"&dsbm="+document.getElementById("dsbm").value+"&dsemail="+encodeURI(escape(document.getElementById("dsemail").value))+"&xid=<%=xid%>&time="+timeParam;			
...
if (!fun.sqlStrCheck(xid))
	return;
String lxbh="0",mc1="",mc2="";
int fflx=0,ffjf=0;
try
{
	strsql="select x.nid,f.ffsj,m1.mmmc as mc1,m2.mmmc as mc2,x.jf,x.fflx,x.lxbh,f.mm1,f.mm2,x.yffjf from tbl_jfffxx x inner join tbl_jfff f on x.jfff=f.nid left join tbl_jfmm m1 on f.mm1=m1.nid left join tbl_jfmm m2 on f.mm2=m2.nid where x.nid="+xid;
	rs=stmt.executeQuery(strsql);
	boolean isGmjf=false;
	if (rs.next())
	{
		fflx=rs.getInt("fflx");
  		  ...
  		  <div class="box">
				...
				if (ffsj!=null && ffsj.length()>0)
				{
					...
					for (int i=0;i<fflxa.length;i++)
					{
						...
						if (fflxa[i].equals("3"))
						{
							...
							out.print("<div class=\"hjrbox1\">发放对象：[可选择全体员工(这里的全体是指部门或者小组的全体)或个别指定的员工进行积分发放]</div>");
				...
				%>
					...
					<div class="delbox" id="xlist1">
						<div class="hjrwrap">
							<div class="hjrbox">
								<div class="hjrbox1">发放对象：[可选择全体员工(这里的全体是指部门或者小组的全体)或个别指定的员工进行积分发放]</div>
				...
				<div class="hjr-box1">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr>
						...
						<td><a id="aisave" href="javascript:void(0);" class="submit" onclick="saveit()" /></a><a href="leaderai.jsp?xid=<%=xid%>" class="reset"></a></td>
					  </tr>
					</table>
				</div>
				</form>
				<%out.print("<script type='text/javascript'>staffn="+staffn+";havejf="+String.valueOf(ffjf)+";");
				if (ffsj!=null && ffsj.length()>0)
				{					
