...
function showljfslist(p)
{
	var timeParam = Math.round(new Date().getTime()/1000);
	var url = "getljfslist.jsp?pno="+p+"&sffsj="+document.getElementById("sffsj").value+"&effsj="+document.getElementById("effsj").value+"&time="+timeParam;	
	  	...
	  	<div class="main2">
				...
				<div class="hjr-box2">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr>
						...
						<td align="right"><span class="caxun" style="margin-right:10px; margin-top:0;" onclick="showljfslist(1)">查询</span></td>
					  </tr>
					</table>
				</div>
				<div class="jfqffjl" id="ljfslist">
					...
					<div class="pages">
						...
						%>
						<a href="javascript:void(0);" <%if (i==1) out.print(" class='psel'"); %> onclick="showljfslist(<%=i%>)"><%=i%></a>
						<%
						if (i>=6) break;
						} %>
						</div>
						<div class="pages-r">
						<%if (pages>1) out.print("<h2><a href='javascript:void(0);' onclick='showljfslist(2)'>下一页</a></h2>");%>					
						</div>		
					</div>
				</div>
				<div style="clear: both; height: 25px; padding-top: 10px; ">您还可以 <a href="leaderjf.jsp" style="color: #2ea6d7">查看积分库存</a></div>
		  </div>
	  	</div>
	</div>
