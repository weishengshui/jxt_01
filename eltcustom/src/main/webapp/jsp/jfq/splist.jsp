...
%>
	...
	<div class="pages-worker">				
		<div class="pages-l">
		<%
		int pageNo = Integer.valueOf(pno);
		for (int i=1;i<=pages;i++) {
		%>
		<a href="javascript:void(0);" <%if (i==pageNo) out.print(" class='psel'"); %> onclick="sspagain(<%=i%>)"><%=i%></a>
		<%
		if (i>=6) break;
		} %>
		</div>
		<div class="pages-r">
		<%
		if (pageNo>1) out.print("<h2><a href='javascript:void(0);' onclick='sspagain("+(pageNo-1)+")'>上一页</a></h2>");
	    if (pageNo<pages) out.print("<h2><a href='javascript:void(0);' onclick='sspagain("+(pageNo+1)+")'>下一页</a></h2>");
	    %>
		</div>		
	</div>
