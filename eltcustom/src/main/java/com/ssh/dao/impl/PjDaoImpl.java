...
public class PjDaoImpl extends PageDaoImpl implements PjDao{
	...
	public String pageSql(String param) {
		String sql = "SELECT t.nid,t.spl,t.yg,t.zpf,t.pjxj,t.pjnr,t.pj,u.ygxm,u.nc, DATE_FORMAT(t.rq,'%Y.%m.%d %H:%i') pjrq FROM tbl_pj t  LEFT JOIN tbl_qyyg u ON u.nid = t.yg WHERE 1=1 "
				+ param + " ORDER BY t.rq DESC";
		return sql;
	}
	public String countSql(String param) {
		String sql = "SELECT COUNT(t.nid) AS count FROM tbl_pj t where 1=1 "
				+ param;
		return sql;
	}
	...
	public List<Map<String, Object>> getSumPf(String param) {
		String sql = " SELECT COUNT(t.nid) AS totalcount,SUM(t.zpf) AS sumxj FROM tbl_pj t where 1=1 "
				+ param;
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		try {
			list = sqldao.query(sql);
